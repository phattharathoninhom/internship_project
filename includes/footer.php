<footer class="main-footer">
    <div class="footer-content">
        <div class="footer-info">
            <h4 class="dept-title">หลักสูตรสารสนเทศศึกษา (Information Studies)</h4>
            <p>ภาควิชาบรรณารักษศาสตร์และสารสนเทศศาสตร์</p>
            <p>คณะมนุษยศาสตร์ มหาวิทยาลัยศรีนครินทรวิโรฒ</p>
            <p class="address"><i class="fa-solid fa-location-dot"></i> เลขที่ 114 อาคาร 2 ชั้น 3 ห้อง 236 ถนนสุขุมวิท 23 แขวงคลองเตยเหนือ เขตวัฒนา กรุงเทพฯ 10110</p>
        </div>
        
        <div class="footer-contact">
            <h4>ติดต่อสอบถาม</h4>
            <p><i class="fa-solid fa-phone"></i> 02-649-5000 ต่อ 16508</p>
            <p><i class="fa-solid fa-envelope"></i> is@g.swu.ac.th</p> </div>

        <div class="footer-social">
            <h4>ติดตามข่าวสาร</h4>
            <div class="social-icons">
                <a href="https://www.facebook.com/isswuofficial" target="_blank"><i class="fa-brands fa-facebook-f"></i></a>
                <a href="/internship_project/index.php" target="_blank"><i class="fa-solid fa-globe"></i></a>
            </div>
        </div>
    </div>
    
    <div class="footer-bottom">
        <p>&copy; <?php echo date("Y"); ?> Information Studies, SWU. All Rights Reserved.</p>
    </div>
</footer>